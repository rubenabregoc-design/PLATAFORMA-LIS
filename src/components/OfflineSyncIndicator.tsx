...
export const OfflineSyncIndicator: React.FC = () => {
  ...
  return (
    <div className="relative">
      ...
      >
        {isSyncing ? (
          <RefreshCw className="w-3.5 h-3.5 text-teal-400 animate-spin shrink-0" />
        ) : !isOnline ? (
        ...
        <span className="hidden 2xl:inline font-mono text-[10.5px]">
          {!isOnline
            ? 'OFFLINE'
            : isSyncing
            ? 'SINCRONIZANDO'
        ...
        {pendingCount > 0 && (
          <span
            className={`text-[9px] font-black font-mono px-1.5 py-0.2 rounded-full ${
        ...
        <ChevronDown className={`w-3 h-3 text-slate-400 transition-transform hidden sm:inline ${isOpen ? 'rotate-180' : ''}`} />
      </button>
