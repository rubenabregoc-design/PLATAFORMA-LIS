...
export interface LisTestParameter { id: string; code: string; loincCode: string; name: string; unit: string; resultType: 'NUMERIC' | 'QUALITATIVE' | 'TEXT' | 'MICROBIOLOGY' | 'PATHOLOGY'; astmMappingCode?: string; hl7MappingCode?: string; referenceRanges: LisReferenceRange[]; criticalValue?: LisCriticalValue; }
...
const MASTER_TESTS: LisMasterTest[] = [
  ...
  { id: 'test-qcl-lip', code: 'QCL-030', loincCode: '24331-1', loincName: 'Lipid Panel', name: 'Perfil Lipídico', shortName: 'Perfil Lipídico', synonyms: ['Lípidos'], areaId: 'QCL', areaName: 'Química Clínica', subarea: 'Perfil Lipídico', resultType: 'PANEL_MULTI_COMPONENT', specimenType: 'Suero', tubeType: 'SST_AMARILLO', tubeColorHex: '#eab308', tubeAdditive: 'Gel', minVolumeMl: 5.0, method: 'Enzimático', fastingRequired: true, tatHours: 4, priceUsd: 35.00, stabilityDetails: '48h 2-8°C', rejectionCriteria: 'Hemólisis', active: true, parameters: [
    { id: 'p-col', code: 'COL', loincCode: '2093-3', name: 'Colesterol Total', unit: 'mg/dL', resultType: 'NUMERIC', referenceRanges: [] },
    { id: 'p-trig', code: 'TRIG', loincCode: '2571-8', name: 'Triglicéridos', unit: 'mg/dL', resultType: 'NUMERIC', referenceRanges: [] },
    { id: 'p-hdl', code: 'HDL', loincCode: '2085-9', name: 'HDL', unit: 'mg/dL', resultType: 'NUMERIC', referenceRanges: [] },
    { id: 'p-ldl', code: 'LDL', loincCode: '2089-1', name: 'LDL', unit: 'mg/dL', resultType: 'NUMERIC', referenceRanges: [] }
  ]},
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-1 bg-slate-950/50 p-1.5 rounded-2xl border border-white/5 overflow-x-auto no-scrollbar shrink-0">
        {[
          ...
          { id: 'PANEL_INSPECTOR', label: 'Paneles', icon: Layers },
      ...
      {activeTab === 'PANEL_INSPECTOR' && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-in fade-in duration-500">
          {tests.filter(t => t.resultType === 'PANEL_MULTI_COMPONENT').map(panel => (
            <div key={panel.id} className="bg-slate-900/60 border border-white/5 p-6 rounded-[2.5rem] space-y-4 hover:border-teal-500/30 transition-all group">
              ...
              <div className="space-y-1.5 pt-2 border-t border-white/5">
                {panel.parameters.map(p => (
                  <div key={p.id} className="flex items-center justify-between text-[10px] text-slate-400 py-1 border-b border-white/5 last:border-0">
                    <span className="font-bold">{p.name}</span>
                    <span className="font-mono text-slate-600">{p.unit}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {activeTab === 'REFERENCE_RANGES' && (
        <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden animate-in fade-in duration-500 shadow-2xl">
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {tests.flatMap(t => t.parameters.length > 0 ? t.parameters.map(p => ({ testName: t.name, param: p })) : [{ testName: t.name, param: { name: 'Directo', referenceRanges: [{ sex: 'AMBOS', ageMinYears: 0, ageMaxYears: 99, minValue: 0, maxValue: 0, unit: t.defaultUnit || 'N/A', source: 'Manual' }] } }]).map((row, idx) => (
                <React.Fragment key={idx}>
                  {row.param.referenceRanges.map((r, rIdx) => (
                    <tr key={`${idx}-${rIdx}`} className="hover:bg-white/[0.02] transition-colors">
