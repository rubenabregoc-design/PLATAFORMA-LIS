...
export const AnalyzerHomologation: React.FC<AnalyzerHomologationProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* MODALS */}
      {/* Resto de modales se mantiene intacto... */}
      {isAnalyzerModalOpen && (
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-8 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3 sticky top-0 bg-slate-900 z-20">
               <h3 className="font-bold text-base text-white">{editingMapping ? 'Editar Mapeo de Traducción' : 'Nuevo Mapeo de Diccionario'}</h3>
            ...
            <form onSubmit={handleSaveMapping} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3"><div className="space-y-1"><label className="font-bold text-slate-300">Tipo Muestra:</label><select value={formData.sampleType} onChange={e => setFormData({ ...formData, sampleType: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white outline-none"><option value="Suero">Suero</option><option value="Sangre Total EDTA">Sangre Total EDTA</option></select></div><div className="space-y-1"><label className="font-bold text-slate-300">Factor de Corrección:</label><input type="number" step="0.01" value={formData.multiplierFactor} onChange={e => setFormData({ ...formData, multiplierFactor: parseFloat(e.target.value) || 1.0 })} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white font-mono outline-none" /></div></div>
              <div className="pt-2 flex justify-end gap-2 border-t border-slate-800"><button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-slate-800 text-slate-300 rounded-xl font-bold transition">Cancelar</button><button type="submit" className="px-5 py-2 bg-teal-500 text-slate-950 font-black rounded-xl transition shadow-lg shadow-teal-500/20">Guardar Diccionario</button></div>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-2xl w-full p-8 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <form onSubmit={handleAddRangeRule} className="p-4 bg-slate-950 border border-slate-800 rounded-2xl space-y-3 text-xs"><div className="grid grid-cols-3 gap-3"><div><label className="text-slate-400 block mb-1 font-bold">Sexo Demográfico:</label><select value={rangeForm.gender} onChange={e => setRangeForm({ ...rangeForm, gender: e.target.value as any })} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2.5 py-2 text-white outline-none"><option value="Ambos">General (Ambos)</option><option value="Masculino">Masculino</option><option value="Femenino">Femenino</option></select></div><div><label className="text-slate-400 block mb-1 font-bold">Límite Mín (Normal):</label><input type="number" step="0.01" value={rangeForm.minValue} onChange={e => setRangeForm({ ...rangeForm, minValue: parseFloat(e.target.value) })} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2.5 py-2 text-emerald-400 font-mono outline-none" /></div><div><label className="text-slate-400 block mb-1 font-bold">Límite Máx (Normal):</label><input type="number" step="0.01" value={rangeForm.maxValue} onChange={e => setRangeForm({ ...rangeForm, maxValue: parseFloat(e.target.value) })} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2.5 py-2 text-emerald-400 font-mono outline-none" /></div></div><div className="pt-2 flex justify-end"><button type="submit" className="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-teal-400 font-black rounded-xl transition">+ Agregar Regla</button></div></form>
            </div>
          </div>
        </div>
      )}
