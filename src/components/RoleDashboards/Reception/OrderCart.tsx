...
import {
  X, Zap, Clock, CheckCircle2, UserPlus, Stethoscope, Search, Sparkles, AlertCircle
} from 'lucide-react';
      ...
      <div className="bg-gradient-to-b from-slate-900 to-slate-950 border border-white/5 rounded-[2.5rem] p-6 shadow-2xl flex flex-col relative group">
         ...
         <div className="flex-1 space-y-6 relative z-10 overflow-y-auto">
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                 <div className="flex items-center space-x-2"><div className="w-6 h-6 bg-teal-500/20 rounded-lg flex items-center justify-center text-teal-400 font-black text-xs">{selectedTests.length + selectedPackages.length}</div><span className="text-[11px] font-black text-white uppercase tracking-widest">Orden Paciente</span></div>
                 {(selectedTests.length > 0 || selectedPackages.length > 0) && (
                    <button onClick={() => { setSelectedTestIds([]); setSelectedPackageIds([]); }} className="text-[8px] font-black text-rose-500/40 hover:text-rose-500 uppercase">Limpiar</button>
                 )}
              </div>
              <div className="space-y-2 pr-1">
                 {/* Render Packages First */}
                 {selectedPackages.map(p => (
                   <div key={p.id} className="flex items-center justify-between p-3 bg-amber-500/10 border border-amber-500/20 rounded-2xl animate-in slide-in-from-right-4 transition-all">
                      <div className="min-w-0 flex-1 pr-2">
                         <div className="flex items-center space-x-1.5">
                            <Sparkles className="w-2.5 h-2.5 text-amber-500" />
                            <div className="text-[9px] font-black text-white uppercase truncate">{p.name}</div>
                         </div>
                         <div className="text-[8px] text-amber-400 font-mono mt-0.5">${p.price.toFixed(2)}</div>
                      </div>
                      <button onClick={() => setSelectedPackageIds(prev => prev.filter(id => id !== p.id))} className="w-6 h-6 flex items-center justify-center bg-slate-950 hover:bg-rose-500 text-slate-700 hover:text-white rounded-lg transition-all shadow-xl"><X className="w-3.5 h-3.5" /></button>
                   </div>
                 ))}
         ...
         <div className="pt-6 border-t border-white/10 space-y-6 relative z-10 shrink-0">
            <div className="flex flex-col items-center">
               ...
               {/* Smart Patient Preparation Guide */}
               {selectedTests.length > 0 && (
                 <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-2xl space-y-3">
                   <div className="flex items-center space-x-2">
                     <AlertCircle className="w-4 h-4 text-amber-500" />
                     <span className="text-[9px] font-black text-amber-400 uppercase tracking-widest">Preparación Sugerida</span>
                   </div>
                   <ul className="space-y-2">
                     {selectedTests.some(t => t.category === 'QUIMICA' || t.category === 'INMUNOLOGIA') && (
                       <li className="text-[8px] text-slate-300 font-bold uppercase leading-tight">• Ayuno estricto de 10-12 horas requerido.</li>
                     )}
                     {selectedTests.some(t => t.id === 'test-lipidico') && (
                       <li className="text-[8px] text-slate-300 font-bold uppercase leading-tight">• No ingerir alcohol 24h antes.</li>
                     )}
                     <li className="text-[8px] text-slate-300 font-bold uppercase leading-tight">• No suspender medicamentos de presión/corazón.</li>
                   </ul>
                   <button className="w-full py-2 bg-amber-500/20 hover:bg-amber-500 text-amber-500 hover:text-slate-950 text-[8px] font-black uppercase rounded-lg transition-all">Imprimir Guía</button>
                 </div>
               )}

            <button
              onClick={handleCreateOrderSubmit}
              disabled={(!foundPatient && !isRegistering) || (selectedTests.length === 0 && selectedPackages.length === 0)}
