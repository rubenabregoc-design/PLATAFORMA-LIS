...
export const TechMedDashboard: React.FC<TechMedDashboardProps> = ({ results, orders, analyzers }) => {
  ...
  const activeAnalyzers = analyzers.filter(a => a.status === 'ONLINE' || (a.status as any) === 'En línea').length;
          ...
          <div className="space-y-3">
            {analyzers.map((a) => (
              <div key={a.id} className="bg-slate-900/40 border border-slate-800 p-4 rounded-3xl flex items-center space-x-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${a.status === 'ONLINE' || (a.status as any) === 'En línea' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
